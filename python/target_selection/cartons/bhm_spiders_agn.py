...
from sdssdb.peewee.sdss5db.catalogdb import (
    ...
    Legacy_Survey_DR8,
    BHM_eFEDS_Veto,
    # CatalogToSDSS_DR16_SpecObj, # <-- in v0.5 we rely on a q3c match instead
    SDSS_DR16_SpecObj,
    EROSITASupersetAGN,
    SDSSV_BOSS_SPALL,
)
...
class BhmSpidersAgnLsdr8Carton(BaseCarton):
    ...
    WHERE
        ...
        AND (ls.fiberflux_r > 1.0 OR ls.fiberflux_z > 2.51189)
        AND ls.fiberflux_r < 3981.07
        AND c.version_id = 21
        AND q3c_radial_query(c.ra,c.dec,150.0,2.0,1.0)
    ORDER BY ls.ls_id, sV.sn_median_all DESC
    LIMIT 20;
    ...
    def build_query(self, version_id, query_region=None):
        ...
        fiberflux_r_min_cadence1 = AB2nMgy(self.parameters['fibermag_r_for_cadence1'])
        fiberflux_r_min_cadence2 = AB2nMgy(self.parameters['fibermag_r_for_cadence2'])
        ...
        priority = (p_f + priority_1 + priority_2 + priority_3).alias("priority")
        ...
        cadence = peewee.Case(
            None,
            (
                (ls.fiberflux_r > fiberflux_r_min_cadence1, cadence1),
                (ls.fiberflux_r > fiberflux_r_min_cadence2, cadence2),
            ),
            cadence3)

        # need to decide if we want to switch between psfmags and fibermags depending on
        # ls.type parameter (PSF or extended)

        # Notes on conversion from ls_fibermags
        ...
        # # for v0.5 we will just use the native ls_fibermag values and will convert them later on.
        ...
        # magnitude_i = (22.5 - 2.5 * fn.log10(fn.greatest(flux30, 0.5 * (ls.fiberflux_r +
        #                                                          ls.fiberflux_z)))).cast('float')
        # opt_prov = peewee.Value('ls_fibermag').cast('string').alias('opt_prov')
        ...
        magnitude_g = peewee.Case(
            ...
            (22.5 - 2.5 * fn.log10(fn.greatest(flux30, ls.fiberflux_g))).cast('float'))

        magnitude_r = peewee.Case(
            ls.type,
            (('PSF', (22.5 - 2.5 * fn.log10(fn.greatest(flux30, ls.flux_r))).cast('float')),),
            (22.5 - 2.5 * fn.log10(fn.greatest(flux30, ls.fiberflux_r))).cast('float'))

        magnitude_z = peewee.Case(
            ls.type,
            (('PSF', (22.5 - 2.5 * fn.log10(fn.greatest(flux30, ls.flux_z))).cast('float')),),
            (22.5 - 2.5 * fn.log10(fn.greatest(flux30, ls.fiberflux_z))).cast('float'))
        ...
        magnitude_i = peewee.Case(
            ...
            (22.5 - 2.5 * fn.log10(
                fn.greatest(flux30, 0.5 * (ls.fiberflux_r + ls.fiberflux_z)))).cast('float'))
        ...
        query = (
            ...
            .distinct([ls.ls_id])   # avoid duplicates - we trust the ls_id
        )
