...
# 
#    #step not shown: using gaia+2mass crossmatch primary key to link gaia+2mass counterparts.
# 
...
#     bp_rp_excess = bp - rp - 0.725   # 0.725 = mode bp-rp color of eBOSS flux standards in SDSS footprint
# 
#     #only deredden if bp_rp_excess is positive
#     worth_dereddening = np.where( bp_rp_excess >= 0)
# 
#     #calculate a_g and a_k using coefficients from Wang & Chen 2019  (use 0 otherwise; np.zero commands truncated for brevity)
...
# 
# meet_std_criteria = np.where( ( ( (gaia_DR2.phot_bp_mean_mag - gaia_DR2.phot_rp_mean_mag) >= 0.65) &
...
# 
# meet_reddened_criteria = np.where( ( ((gaia_DR2.phot_g_mean_mag - ag) - (twomass_psc.k_m - ak)) >= 1.1) &
