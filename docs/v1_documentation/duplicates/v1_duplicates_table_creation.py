...
class Duplicates:
    ...
    def run_query1(self, model, rel_model):
        ...
        cte_catalogid = (
            rel_model.select(rel_model.catalogid, fn.COUNT(rel_model.target_id))
        ...
        cte_targetid = (
            rel_model.select(rel_model.target_id, fn.COUNT(rel_model.catalogid))
            .where(rel_model.version_id == self.version_id)
        ...
        query1 = (
            rel_model.select(
                ...
                rel_model.distance,
                rel_model.best,
                Case(None, [(cte_catalogid.c.catalogid.is_null(), False)], True).alias(
                    "dup_targetid"
                ),
                Case(None, [(cte_targetid.c.target_id.is_null(), False)], True).alias(
                    "dup_catalogid"
                ),
            )
            .join(
        ...
        tf = time.time()
        self.log.info(
            f"The first query took {(tf-ti):.2f} seconds and selected {n_ins1:,} " "duplicates"
        )
    ...
    def run_query2(self, model, rel_model):
        ...
        q2 = dup1m.select(
            ...
            Case(
                None,
                [
                    ((dup1m.dup_catalogid >> True) & (dup1m.distance.is_null()), "phase_1"),
                    (
                        (
                            ...
                            & (
                                ...
                                & (
                                    fn.first_value(dup1m.distance).over(
                                        partition_by=[dup1m.catalogid],
                                        order_by=[dup1m.distance.asc()],
                                    )
                                    == dup1m.distance
                                )
                            )
                ...
                "None",
            ).alias("dup_type_catid"),
            Case(
                None,
                [
                    ((dup1m.dup_targetid >> True) & (dup1m.distance.is_null()), "phase_1"),
                    (
                        (
    ...
    def process_table(self, tablename):
        ...
        dup_final_fields = [
            dup_final_model._meta.fields[field] for field in dup_final_model._meta.fields
        ]
        ins_query = dup_final_model.insert_from(
            dup2m.select(
                dup2m.input_table,
                dup2m.catalogid,
                dup2m.target_id,
        ...
        tf = time.time()
        self.log.info(
            f"Completely processed {tablename} in {(tf-ti):.2f} seconds "
    ...
    def run(self, starting_table=None, ending_table=None, overwrite=True):
        ...
        rel_models = [
            self.database.models[f"catalogdb.catalog_to_{name}"] for name in tables_to_process
        ]
        ...
        self.log.info(" ")
        self.log.info(
            f"Finished checking duplicates ingesting {total_entries:,} total entries "
