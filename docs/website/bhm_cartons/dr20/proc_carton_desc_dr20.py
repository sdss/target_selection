...
with open(outfilehtml, "wt") as of:
    for c in j["cartons"]:
        ...
        cadences = c["cadences"].replace("{", "").replace("}", "")
        priority = c["priority"].replace("{", "").replace("}", "")
        code = c.get("code", "n/a")
        if code != "n/a":
            this_url_ts = f'{url_ts}/blob/{tag}/python/target_selection/cartons/{c["code"]}'
            implementation = f'<a href="{this_url_ts}">{code}</a>'
        else:
            implementation = code
        ...
        s = [
            ...
            f"<p><strong>crossmatch plan:</strong> {crossmatch}</p>",
            f'<p><strong>Summary:</strong> {c["summary"]}</p>',
            f'<p><strong>Simplified description of selection criteria:'
            f'</strong> {c["selection"]}</p>',
            f'<p><strong>Catalogdb tables required:</strong> <tt>{c["tables"]}</tt></p>',
            f'<p><strong>Target priority options:</strong> {priority}</p>',
            f'<p><strong>Cadence options:</strong> <tt>{cadences}</tt></p>',
            f'<p><strong>Implementation:</strong> {implementation} </p>' ,
